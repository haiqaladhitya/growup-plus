...
st.markdown(f"""
<style>
  .title {{ color: {theme['primary']}; text-align: center; }}
</style>
<h1 class="title">👶 GrowUp+ – Pemantauan Tumbuh Kembang Anak</h1>
""", unsafe_allow_html=True)

# ─── 5. INPUT SIDEBAR DENGAN FORM ─────────────────────────────────────────────
...
# ─── 6. LOGIKA PREDIKSI & TAMPILKAN HASIL ────────────────────────────────────
if submitted:
    # Encode gender & siapkan X
    ...
    with col2:
        st.subheader("⚖️ Hasil Prediksi Wasting")
        st.markdown(f"**{labs_w[pw]}**")
        st.progress((pw+1)/4, text="Tingkat Risiko")
        
        # Pie chart Berat Ideal vs Aktual
        berat_ideal = berat * 1.1  # misal 10% di atas berat aktual
        df2 = pd.DataFrame({
    ...
    st.info("Silakan isi data anak di sidebar, lalu klik **Mulai Analisis** untuk melihat hasil.")
